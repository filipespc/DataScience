...
import numpy as np
degrees_zero = (np.asarray(fahrenheit_degrees) + 459.67).tolist()
...
smoking_map = {"none": 0, "a few": 1, "some": 2, "a lot": 3}
smoking_numbers = [smoking_map[response] for response in survey_responses]

average_smoking = sum(smoking_numbers)/len(smoking_numbers)
...
male_savings = savings_array[gender_array == "male"].mean()
female_savings = savings_array[gender_array == "female"].mean()
