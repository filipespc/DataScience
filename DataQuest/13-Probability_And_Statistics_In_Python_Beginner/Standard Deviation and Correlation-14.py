...
point_variance = 0
for each in nba_stats["pts"]:
    point_variance += (each - pts_mean)**2
...
def std_dev(vector):
    ...
    v_mean = vector.mean()
    for each in vector:
        std_dev_r += (each - v_mean)**2
    std_dev_r = (std_dev_r/len(vector))**(1/2)
    return std_dev_r
...
probabilities_2 = norm.pdf(points_2, 0, 2)

plt.plot(points_2, probabilities_2)
plt.show()
...
import numpy as np
wing_lengths_array = np.array(wing_lengths)
mean = wing_lengths_array.mean()
std_dev = wing_lengths_array.std()

std_array = (wing_lengths - mean)/std_dev

within_one_percentage = ((std_array <= 1) & (std_array >= -1)).sum()/len(std_array)
within_two_percentage = ((std_array <= 2) & (std_array >= -2)).sum()/len(std_array)
within_three_percentage = ((std_array <= 3) & (std_array >= -3)).sum()/len(std_array)
