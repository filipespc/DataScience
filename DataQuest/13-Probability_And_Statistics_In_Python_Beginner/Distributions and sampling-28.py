...
high_pop_counties = income[income["pop_over_25"] > 500000]
lowest_income_high_pop_county = high_pop_counties["county"][high_pop_counties["median_income"].idxmin()]
...
random.seed(1)
plt.hist([roll() for _ in range(100)], 6)
plt.show()

random.seed(1)
plt.hist([roll() for _ in range(10000)], 6)
plt.show()
...
large_sample_mean = numpy.asarray(large_sample).mean()

deviations_from_mean = abs(0.18 - large_sample_mean)/large_sample_std
over_18_count = (numpy.asarray(large_sample) >= 0.18).sum()
...
random_samples = []
for i in range(1000):
    random_samples.append(select_random_sample(100)["median_income"].mean())
...
print((income["median_income_hs"]/income["median_income_college"]).mean())
...
import numpy as np
significance_value = (np.asarray(mean_ratios) >= .675).sum()/len(mean_ratios)
