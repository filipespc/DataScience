...
predicted_quality = wine_quality["density"].apply(lambda x: slope*x + intercept)
rss = ((wine_quality["quality"] - predicted_quality)**2).sum()
...
std_err = (rss/(wine_quality.shape[0]-2))**(1/2)

def within_err(y, y_pred, std_err, num_std_err):
    return (abs(y - y_pred) <= std_err*num_std_err).sum()/len(y)

within_one = within_err(wine_quality["quality"], predicted_y, std_err, 1)
within_two = within_err(wine_quality["quality"], predicted_y, std_err, 2)
within_three = within_err(wine_quality["quality"], predicted_y, std_err, 3)