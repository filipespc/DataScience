...
# Make a GET request to the GitHub API with our headers.
# This API endpoint will give us the orgnizations to which Vik Paruchuri belongs to.
response = requests.get("https://api.github.com/users/VikParuchuri/orgs", headers=headers)

# Print the content of the response. As you can see, this token corresponds to the organiozations Vik Paruchuri belongs to.
orgs = response.json()
print(orgs)
