...
days_over_threshold = (bikes["cnt"] > 4000).sum()
...
def prob_k_in_n(p,k,n):
    return comb_k_in_n(k,n) * (p**k) * ((1-p)**(n-k))

prob_8 = prob_k_in_n(p,8,10)
prob_9 = prob_k_in_n(p,9,10)
prob_10 = prob_k_in_n(p,10,10)